...
if (!firebaseReady) { $("#setup-warning").hidden=false; $("#login-form button").disabled=true; }
  ...
  onAuthStateChanged(auth, async(user)=>{
    ...
    $("#login-panel").hidden=!!user; $("#editor").hidden=!user; $("#logout").hidden=!user; if(user) await load();
  });
}
...
$(".add-item").addEventListener("click",()=>{state.social.push({url:"",imageUrl:"",title:"",caption:"",visible:true});render()});
...
function renderSocial(){$("#social-fields").innerHTML=state.social.map((v,i)=>item(`Social post ${i+1}`,`<button type="button" class="remove" data-remove="${i}">Remove</button><div class="grid"><label>Instagram post URL<input data-path="social.${i}.url" value="${esc(v.url)}"></label><label>Display image URL<input data-path="social.${i}.imageUrl" value="${esc(v.imageUrl)}"></label><label>Title<input data-path="social.${i}.title" value="${esc(v.title)}"></label><label>Caption<input data-path="social.${i}.caption" value="${esc(v.caption)}"></label></div><label>Or upload image<input type="file" accept="image/*" data-upload="social.${i}.imageUrl"></label><label class="check"><input type="checkbox" data-path="social.${i}.visible" ${v.visible!==false?"checked":""}> Show on website</label>`)).join("");document.querySelectorAll("[data-remove]").forEach(b=>b.onclick=()=>{state.social.splice(+b.dataset.remove,1);render()})}
